...
export default function Slider({
  ...
  const renderProjectSlide = (project: Project, isActive: boolean) => (
    <div
      className={`w-[min(90vw,36rem)] group bg-card border rounded-xl p-6 flex flex-col gap-4 transition-colors ${
 ...
 <a 
  ...
  const renderSkillSlide = (
    ...
    <div
      className={`w-[min(90vw,36rem)] bg-card border rounded-xl p-6 flex flex-col gap-4 transition-colors ${
        isActive ? "border-accent" : "border-border"
      }`}
    >
  ...
  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 pointer-events-none">
    {items.map((_, index) => (
      <button
        ...
        className={`pointer-events-auto w-2.5 h-2.5 rounded-full border border-border transition-colors ${
          index === currentIndex ? "bg-accent border-accent" : "bg-card hover:border-accent"
        }`}
